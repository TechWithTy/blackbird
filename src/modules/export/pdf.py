...
async def save_to_pdf(foundAccounts, resultType, config):
    ...
    try:
        registerFont(TTFont(config.FONT_NAME_REGULAR, regularFontFile))
        ...
        _generate_pdf_content(canva, foundAccounts, resultType, config, width, height)
        canva.save()
...
def _generate_pdf_content(canva, foundAccounts, resultType, config, width, height):
    ...
    if getattr(config, "ai_analysis", None):
        # Background for the AI summary section
    ...
    if accountsCount >= 1:
        ...
        canva.drawImage(
            ...
            40,
            height - 245 - content_y_offset,
            width=12,
            height=12,
            mask="auto",
        )
        canva.drawString(55, height - 245 - content_y_offset, f"Results ({accountsCount})")

        y_position = height - 270 - content_y_offset
        for result in foundAccounts:
            if y_position < 72:
                canva.showPage()
                if getattr(config, "ai_analysis", None) and config.ai_analysis.get("summary"):
                ...
                if result.get("metadata"):
                    ...
                    for data in result["metadata"]:
                        if data["type"] == "String":
                            ...
                            canva.setFont(config.FONT_NAME_REGULAR, 7)
                            canva.drawString(
                                93 + nameWidth, y_position, f"{data['value']}"
                            )
                            y_position -= 15
